...
def _db_get_group_config(group_id: int) -> dict | None:
    ...
    try:
        response = _supabase_client.table("allowed_groups")\
            .select("group_id, group_name, merchant_name, slipok_mode, allowed_accounts, min_limit, max_limit, slipok_min_amount, slipok_keys")\
        ...
        try:
            ...
            if response.data:
                ...
                for key in ["min_limit", "max_limit", "slipok_min_amount", "slipok_keys"]:
                    data[key] = None
                return data
            return None
...
async def update_group_config(group_id: int, merchant_name: str | None = None, slipok_mode: str | None = None,
                              ...
                              max_limit: float | str | None = None, slipok_min_amount: float | str | None = None,
                              slipok_keys: str | None = None) -> bool:
    ...
    # slipok_keys: comma-separated branch_ids, "default" to reset to global
    if slipok_keys is not None:
        updates["slipok_keys"] = slipok_keys if slipok_keys.lower() != "default" else None
